...
/**
 * Delete-post resolves with nothing. The API answers 204, which carries no body,
 * and the document now says so: the 204 response declares no content at all. This
 * alias used to be derived from a JSON body the document declared and the API
 * never sent, so it described a value no caller could ever receive.
 */
export type PostDeleteResult = void;
